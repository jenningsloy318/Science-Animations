...
(function () {
  ...
  function build(renderer) {
    ...
    const shellMats = cut.mats;         /* one material per shell, radial order */
    ...
    state.cutGroup = cut.cutGroup;      /* rotated each frame to face the camera */
    const matsTrack = {};
    ...
    function addTower(phi, r, energy, em, eta) {
      ...
      m.position.copy(dir.clone().multiplyScalar(r + h / 2 + 0.06));   /* base sits ON the shell */
    ...
    function startSequence(seed, forcedType) {
      ...
      for (const t of ev.towers) {
        addTower(t.phi, t.em ? 8.3 : 11.0, t.e, t.em, t.eta);
        info.sumET += t.e;
      }
    ...
    function update(dt, orbit, active) {
      ...
      /* cutaway opening always faces the camera (no back-arc peeking) */
      if (state.cutGroup && orbit && orbit.cur) state.cutGroup.rotation.y = orbit.cur.az - Math.PI / 2;
      /* idle orbit drift */
      if (orbit && orbit.idleT > 4) orbit.des.az += dt * 0.06;
    }
