...
用四个 3D 视图把光合作用讲清楚：发生在哪（放大之旅）→ 光反应怎么把光变成"电池"→ 卡尔文循环怎么花电池拼糖 → 用已核实的数字看真相。

## 四个视图

| 视图 | 内容 |
| --- | --- |
| ① 叶子里 | 叶片横切 → 叶肉细胞 → 叶绿体三级放大；看清基粒（类囊体垛）和基质 |
| ② 光反应 | 类囊体膜横截面：光子 → PSII/PSI → 电子 Z 链 → H⁺ 入腔 → ATP 合酶旋转；Kok 循环 S0–S4 每 4e⁻ 放一个 O₂；实时计数 O₂/NADPH/ATP |
| ③ 合成循环 | 卡尔文循环赛道：固定(RuBisCO) → 还原(花 3ATP+2NADPH) → 再生；每 3 圈净得 1 G3P，6 G3P = 1 葡萄糖 |
